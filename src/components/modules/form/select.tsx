import { Select, SelectItem, SelectValue, SelectTrigger, SelectContent, SelectGroup } from "@/components/ui/select"
...
export function FormSelect<T extends FieldValues>({
	...
	const { field: test, fieldState, formState } = useController({ control, name })
	return (
		<div className="flex flex-col gap-2 relative pb-4">
			...
			/>
			{fieldState.error && (
				<p className="text-red-500 text-[13px] absolute -bottom-1.5 right-0">{fieldState.error.message}</p>
			)}
		</div>
	)
}
